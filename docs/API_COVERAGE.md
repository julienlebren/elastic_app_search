...
This package targets the App Search public API (including crawler and adaptive relevance APIs).
...
| Adaptive relevance | `adaptive_relevance/suggestions` (`GET`, `POST`, `PUT`), `adaptive_relevance/suggestions/{query}` (`GET`, `POST`), `adaptive_relevance/settings` (`GET`, `PUT`), `adaptive_relevance/update_process` (`POST`) | Complete |
